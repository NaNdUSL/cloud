...
public:
    ...
    Point2D operator +(const Point2D &p) const { return {X+p.X, Y+p.Y};}
    Point2D operator *(const float f) const { return {f*X, f*Y};}
    Point2D operator *(const double f) const { return {(float)(f*X), (float)(f*Y)};}
    friend Point2D operator*(const float f, const Point2D& p) {
        return p*f;
    }
    friend Point2D operator*(const double f, const Point2D& p) {
        return p*f;
    }
    ...
    // note that methods declared within the class are inline by default
    Point2D Permute(int x, int y) {
        const float XY[3]={X,Y};
        return Point2D(XY[x], XY[y]);
    }
};
